# book blob creation and in frame (fast) manipulation, verification
# book class divider and primitive tags
...
# Helper functions
def interpol_shape_annot(shape_buff, interpol, ):
    """
    We wont work with box collapsing here
    
    two special cases
    if an edge of the bb(bounding box) is at the frame edge
    - and the bb is quiting the frame
    -- (simple case and 
    - or comming into the frame
    -- we will keep it at edge with a little 

    """

def save_buffer_to_file(buff):
    if type(buff) == type(""):
        with open(rpath+f'{video_file}_frameAnno.txt', 'a') as annof:
            annof.write(buff)
...
if __name__ == "__main__":
    ...
    rpath = 'ModelData/vids/'
    video_file = 'VID_20230824_143549'
    F_ANNOTE = open(rpath+video_file+'.txt')

    # Open video file
    cap = cv.VideoCapture(rpath+video_file+'.mp4')
    ...
    while cap.isOpened():
        ...
        # Enter loop to draw quads on single frame
        key = 0
        while key != 27:
            ...
            if   key == ord('n') or key == ord(' '):
                if len(shapes[0]):
                    ...
                    if BUFFER_SELECT:       
                        ...
                        SHAPE_BUFFER.append([FRAME_COUNTER, shapes])

                    print(annotations)

                # Save the frame annotations to a data structure or file
                """with open(rpath+f'{video_file}_frameAnno.txt', 'a') as annof:
                    annof.write(f'\n{FRAME_COUNTER}_{len(shapes)}')
                    for s in shapes:
                        annof.write(f'\n{len(s)}')
    ...
    save_buffer_to_file(SHAPE_BUFFER)
